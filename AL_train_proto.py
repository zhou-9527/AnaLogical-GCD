...
def train_AL (label_list, AL_model, vis_features_labelled, text_features, args,vis_features_labelled_proto, label_list_proto, pre_train = False):
    tl = Averager()
    model = AL_model.train()
    DR_Loss = DRLoss()

    optimizer = torch.optim.SGD(model.parameters(), args.lr_AL, momentum=0.9, nesterov=True, weight_decay=args.weight_decay_AL)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs_AL, eta_min=args.lr * 0.01)
    ...
    for epoch in range(args.epochs_AL):
        ...
        proto_list_base_f = vis_features_labelled_proto.to(device).detach()
        proto_list_new_f = torch.index_select(vis_features_labelled, 0, torch.tensor(class_list_decoer_new)).to(device).detach()
        text_feature_base_idx = label_list_proto.to(device).detach()
        ...
        num_class_labelled = len(text_feature_new_f)
        num_class_unlabelled = len(text_feature_base_f)
...
if __name__ == "__main__":
    ...
    vis_features_labelled_proto, label_list_proto = get_proto_labelled(train_dataset.labelled_dataset, vis_encoder, args)
    ...
    train_AL(label_list, AL_model, vis_features_labelled, text_features, args, vis_features_labelled_proto,label_list_proto, pre_train=True)
    vis_features_labelled = vis_features_labelled_proto.to(device)
    label_list = label_list_proto
