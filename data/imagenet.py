...
def get_imagenet_100_datasets(train_transform, test_transform, train_classes=range(80),
    ...
    imagenet_training_set = ImageNetBase(root=os.path.join(imagenet_root, 'images/train'), transform=train_transform)
    whole_training_set = subsample_classes(imagenet_training_set, include_classes=subsampled_100_classes)

    with open("/data4/HanJZ/ClipGCD/data/imagenet_classnames.txt", "r") as f:
        imagenet_classes = f.readlines()
    imagenet_classes = [cls.strip().split() for cls in imagenet_classes]
    imagenet100_classes = []
    for cls_index in subsampled_100_classes:
        imagenet100_classes.append(" ".join((imagenet_classes[cls_index])[1:]))
    ...
    test_dataset = ImageNetBase(root=os.path.join(imagenet_root, 'images/val'), transform=test_transform)
    ...
    return all_datasets, imagenet100_classes
