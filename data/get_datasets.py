...
def get_datasets_1(dataset_name, train_transform, test_transform, args):
    ...
    unlabelled_train_all_test = MergedDataset(labelled_dataset=labelled_train_examples_test,
                                              unlabelled_dataset=unlabelled_train_examples_test)

    return train_dataset, test_dataset, unlabelled_train_examples_test, unlabelled_train_all_test, datasets
...
def get_datasets(dataset_name, train_transform, test_transform, args):
    ...
    # Get datasets
    get_dataset_f = get_dataset_funcs[dataset_name]
    datasets, classesname = get_dataset_f(train_transform=train_transform, test_transform=test_transform,
                            train_classes=args.train_classes,
                            prop_train_labels=args.prop_train_labels,
                            split_train_val=False)
    ...
    # change target label with target_transform_dict
    target_transform_dict_new = {v: k for k, v in target_transform_dict.items()}
    classesname_changed = []
    for i in range (len(args.train_classes) + len(args.unlabeled_classes)):
        idx = target_transform_dict_new[i]
        classesname_changed.append(classesname[idx])
    ...
    return train_dataset, test_dataset, unlabelled_train_examples_test, datasets, classesname_changed,unlabelled_train_all_test
